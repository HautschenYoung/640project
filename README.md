...
dust3r requires image frames as input and does not accept video input directly. Therefore, use `preprocess.py` to extract frames from videos. `preprocess.py` can accept 3 arguments and takes videos in `data/` as input and outputs frames into individual folders in `preprocessed_frames/` per video. Zero argument also would work and the preprocess would run in default arguments.
